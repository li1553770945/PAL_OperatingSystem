...
//  memset(disk.pages, 0, sizeof(disk.pages));
	MemsetT<char>(disk.pages,0,sizeof(disk.pages));
  *R(VIRTIO_MMIO_QUEUE_PFN) = ((uint64)disk.pages-PhymemVirmemOffset()) >> PageSizeBit;
...
//    panic("virtio_disk_intr 1");
	kout[Fault]<<"virtio_disk_intr 1"<<endl;
  if(disk.free[i])
//    panic("virtio_disk_intr 2");
	kout[Fault]<<"virtio_disk_intr 2"<<endl;
  disk.desc[i].addr = 0;
  disk.free[i] = 1;
//  wakeup(&disk.free[0]);
	disk.sem->Signal(Semaphore::SignalAll);
}
  ...
  } buf0;
{
  ...
  disk.desc[idx[0]].addr = (PtrInt)&buf0-PhymemVirmemOffset();//??
  disk.desc[idx[0]].len = sizeof(buf0);
  disk.desc[idx[0]].flags = VRING_DESC_F_NEXT;
  disk.desc[idx[0]].next = idx[1];

  disk.desc[idx[1]].addr =(PtrInt)buf-PhymemVirmemOffset();// (uint64) b->data;
  ...
  disk.desc[idx[2]].addr = (uint64) &disk.info[idx[0]].status-PhymemVirmemOffset();
...
//  disk.info[idx[0]].b = b;
	if (disk.info[idx[0]].sem==nullptr)
		disk.info[idx[0]].sem=new Semaphore(0);
...
//      panic("virtio_disk_intr status");
	if (i>=100)
		kout[Fault]<<"virtio_disk_intr status "<<(int)disk.info[id].status<<endl;
